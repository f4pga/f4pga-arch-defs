...
connection database in lue of Project X-Ray."""
    ...
    if args.site_as_tile:
        ...
        import_site_as_tile(db, args, prjxray.site_type.SitePinDirection)
    elif args.connection_database:
        with sqlite3.connect("file:{}?mode=ro".format(
                args.connection_database), uri=True) as conn:
            import_tile_from_database(conn, args, prjxray.site_type.SitePinDirection)
    else:
        import_tile(db, args, prjxray.site_type.SitePinDirection)
