    ...
    if (not is_blackbox) or is_submode:
        ...
        for cname, i_of in mod.cells:
            ...
            for pin, net in inp_cons:
                ...
                if len(drvs) == 0:
                    print("ERROR: pin {}.{} has no driver, interconnect will be missing".format(pb_name, pin))
                    assert False
                elif len(drvs) > 1:
                    print("ERROR: pin {}.{} has multiple drivers, interconnect will be overspecified".format(pb_name, pin))
                    assert False
...
def make_pb_type(mod):
    ...
    if has_modes:
        for mode in modes:
            ...
            mode_yj = YosysJSON(yosys.run.vlog_to_json(args.infiles, flatten=False, aig=False, mode=smode, module_with_mode=mod.name))
