...
def import_tile(db, args, SitePinDirection):
    ...
    if not args.fused_sites:
        ...
        for site in tile.get_sites():
            ...
            for site_pin in site.site_pins:
                ...
                if site_type_pin.direction == SitePinDirection.IN:
                    if site_pin.wire is not None:
                        input_wires.add(site_pin.wire)
                elif site_type_pin.direction == SitePinDirection.OUT:
                    if site_pin.wire is not None:
...
def import_site_as_tile(db, args, SitePinDirection):
...
def find_connections(conn, input_wires, output_wires, tile_type_pkey, SitePinDirection):
        ...
        """, (node_pkey, )
            ...
            for wire_tile_type_pkey, wire_name, site_type_name, site_pin_name, dir, count in cur:
                if wire_tile_type_pkey == tile_type_pkey:
                    ...
                    direction = SitePinDirection(dir)
...
def import_tile_from_database(conn, args, SitePinDirection):
...
WHERE
    site_pkey = ?""", (site_pkey, )):
            direction = SitePinDirection(direction)
    ...
    top_level_connections, internal_connections = find_connections(
        conn, input_wires, output_wires, tile_type_pkey, SitePinDirection
    )
...
WHERE
  ...
  AND wire_in_tile.site_pin_pkey IS NOT NULL;
        ...
        for site_pin_name, site_pin_direction, site_pin_wire in site_pins:
            site_pin_direction = SitePinDirection(
                site_pin_direction
            )
            ...
            if site_pin_direction == SitePinDirection.IN:
                add_direct(
                    interconnect_xml,
                    input=object_ref(
            ...
            elif site_pin_direction == SitePinDirection.OUT:
                pass
            else:
