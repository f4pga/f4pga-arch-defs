...
def add_l_track(graph, x0, y0, x1, y1, segment_id, switch_id, hint=None):
    """
...
def add_track_chain(graph, chan, u, v0, v1, segment_id, switch_id):
    """
    ...
    for v in coords:
        ...
        curr_node_id = graph.add_track(
            track,
            segment_id,
            direction=direction,
            timing=rr.NodeTiming(r=0.0, c=0.0)
        )
        curr_node = graph.nodes[-1]
        assert curr_node.id == curr_node_id
...
def add_tracks_for_const_network(graph, const, tile_grid, src_loc=None, src_pin=None):
    """
    ...
    # Find the source tile if not given
    if src_loc is None:
        src_loc = [
    ...
    # Format the pin name if not given
    if src_pin is None:
        pin_name = "TL-SYN_{const}.{const}0_{const}[0]".format(const=const)
    else:
        pin_name = src_pin.format(const=const)
...
def populate_hop_connections(graph, switchbox_models, connections):
    ...
    for connection in bar(conns):
        ...
        if src_node is None or dst_node is None:
            ...
            if src_node is None:
                print("WARNING: No node for switchbox output pin '{}' at '{}'".format(
                    connection.src.pin, connection.src.loc
                ))

            if src_node is None:
                print("WARNING: No node for switchbox input pin '{}' at '{}'".format(
                    connection.dst.pin, connection.dst.loc
                ))
    ...
    for loc, switchbox_model in bar(switchbox_models.items()):
        ...
        for pin in switchbox_model.switchbox.inputs.values():
            ...
            if pin.name in cand_node_map:
                ...
                sbox_node = switchbox_model.get_input_node(pin.name)
                if sbox_node is None:
                    print("WARNING: No switchbox input pin '{}' found at ({}, {})".format(
                        pin.name,
                        loc.x,
                        loc.y
                    ))
