...
def import_graph_nodes(conn, graph, node_mapping, connection_box_map):
    ...
    for node_idx, node in enumerate(graph.nodes):
        ...
        # look up the physical tile type for IOPADs
        if tile_type == 'IOPAD':
            cur.execute(
                """
SELECT tile_type.name FROM tile
  ...
  WHERE tile.grid_x = ? AND tile.grid_y = ?;
                """, (node.loc.x_low, node.loc.y_low)
            )
            tile_type = cur.fetchone()[0]
...
SELECT site_pkey FROM site_as_tile WHERE pkey = ?;
        ...
        if side_graph_node_pkey is not None:
            ...
            update_connection_box(
                conn, graph, side_graph_node_pkey, node_idx, connection_box_map
            )
