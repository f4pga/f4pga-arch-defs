...
def mark_track_liveness(
        conn, input_only_nodes, output_only_nodes, alive_tracks
):
...
def build_channels(connection_database):
    x_channel_models = {}
    y_channel_models = {}

    with sqlite3.connect('file:{}?mode=ro'.format(connection_database),
                         uri=True) as conn:
        cur = conn.cursor()
