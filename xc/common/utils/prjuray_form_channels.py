#!/usr/bin/env python3
""" Classify US/US+ nodes and generate channels for required nodes.
...
import prjuray.db
import prjuray.tile
from prjuray.grid_types import GridLoc
...
def filter_loc(loc, grid_limit):
    ...
    if loc.grid_x < grid_limit[0] or loc.grid_x > grid_limit[2]:
        return False
    if loc.grid_y < grid_limit[1] or loc.grid_y > grid_limit[3]:
        return False
...
def import_phy_grid(db, grid, conn, get_switch, get_switch_timing, grid_limit=None):
    write_cur = conn.cursor()
...
VALUES
  (?, ?);""", (phy_tile_pkey, wire_in_tile_pkey)
    ...
    for connection in progressbar_utils.progressbar(
        ...
        loc_a = GridLoc(connection.wire_a.grid_x, connection.wire_a.grid_y)
        loc_b = GridLoc(connection.wire_b.grid_x, connection.wire_b.grid_y)
    ...
    print("{} wires, {} nodes".format(len(wires), len(nodes)))
...
SET
  site_wire_pkey = (
    ...
    cur.execute("CREATE INDEX aaa ON pip_in_tile(src_wire_in_tile_pkey, dest_wire_in_tile_pkey, is_pseudo);")
...
def main():
    ...
    parser.add_argument(
        '--grid_limit',
        help='Grid limit (optional). Provide as 4 numbers <x1> <y1> <x2> <y2> (inclusive)',
        type=int,
        nargs=4
    )
    parser.add_argument(
        '--connection_database',
        help='Connection database to be created',
        required=True
    )
    parser.add_argument(
        '--grid_map_output',
    ...
    with DatabaseCache(args.connection_database) as conn:
        ...
        import_phy_grid(db, grid, conn, get_switch, get_switch_timing, args.grid_limit)

        segments = import_segments(conn, db)

        print("{}: Initial database formed".format(datetime.datetime.now()))
        import_nodes(db, grid, conn, args.grid_limit)
