...
def main():
    ...
    # Parse args
    parser = argparse.ArgumentParser(description="Generates top-level verilog for scalable processing test")
    parser.add_argument('--top-name', type=str, default=None)
    parser.add_argument('--num-processing-units', type=int, default=1, required=True)
    ...
    for line in template:
        ...
        if "NUM_PROCESSING_UNITS" in line:
            verilog.append(".NUM_PROCESSING_UNITS   (%d)," % args.num_processing_units)
