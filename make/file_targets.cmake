# This CMake include file defines utility functions for handling generated
# files that are required in another CMake directory other than the one where
# it was generated.
#
# Key functions:
...
#
# When writing targets that rely on files using these functions, never use a
# raw source path, e.g. ${CMAKE_CURRENT_SOURCE_DIR}/example_dir/example.file
# or example.file.  Instead always call GET_FILE_LOCATION on the path.  This
# is because the actual source location may not be in the source tree, but
# instead be in the build tree.
#
# When writing targets that rely on files using these functions, always DEPEND
# on both the file location (from GET_FILE_LOCATION) and the file target
# (from GET_FILE_TARGET).  If you only DEPEND on the file location, the file
# will not be generated the first time through the build.
#
# Utility functions:
#   APPEND_FILE_LOCATION - Appends to a list the file location of specified
#                          file.
#   APPEND_FILE_DEPENDENCY - Appends to a list both the file location and
#                            file target of specified file.
...
function(APPEND_FILE_DEPENDENCY var src_file)
  # Appends to list var in PARENT_SCOPE both file location and file target
  # for given src_file.
  GET_FILE_TARGET(SRC_TARGET ${src_file})
  get_target_property(SRC_LOCATION ${SRC_TARGET} LOCATION)
  if("${SRC_LOCATION}" STREQUAL "NOT_FOUND")
...
function(APPEND_FILE_INCLUDES var src_file)
  # Appends to list var in PARENT_SCOPE both includes listed for file target
  # for given src_file.
  GET_FILE_TARGET(SRC_TARGET ${src_file})
...
function(GET_VERILOG_INCLUDES var file)
  ...
  #
  # Note this function cannot be used at generation time because
  # execute_process is called during configure step and generated files don't
  # exist yet.
  execute_process(
...
function(MAKE_FILE_TARGET)
  ...
  #   )
  #
  # Creates new file target for given source location.  Even if
  # MAKE_FILE_TARGET is being called on a file that is located in the binary
  # directory, always pass the source location it would have if the source and
  # binary directories were the same.  This is important for dependency
  # definitions that assume all inputs are in one folder.
  #
